...
Evaluates the pretrained MSKA-SLT model on 41 conditions: 1 clean + 8 misalignment types x 5 severity levels.
...
# MSKA imports. Note that rename datasets.py to mska_datasets.py 
# to avoid confusion with HF's Datasets library inside evaluate library
...
from misalign import apply_misalignment, get_all_conditions, CONDITION_TYPES, SEVERITY_LEVELS
...
def compute_metrics(results, level='word'): # Compute BLEU-1..4, ROUGE-L, and METEOR (if available)
    ...
    try: # METEOR via HuggingFace evaluate (optional)
        meteor = hf_evaluate.load('meteor')
        score = meteor.compute(predictions=hyps, references=refs)
        metrics['meteor'] = round(score['meteor'] * 100, 2)
...
def run_benchmark(bench_cfg, mska_cfg, model, base_dataset, device): # Iterate over all 41 conditions, run inference, collect metrics
    ...
    for idx, (cond, sev, ds, de) in enumerate(conditions):
        ...
        # store metrics
        if cond == 'clean': all_results['metrics']['clean'] = metrics
        else: all_results['metrics'].setdefault(cond, {})[str(sev)] = metrics
...
def main():
    ...
    if args.clean_only:
        ...
        all_results = run_benchmark(bench_cfg, mska_cfg, model, base_dataset, device)
