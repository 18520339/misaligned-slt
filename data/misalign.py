...
MIN_FRAMES_DEFAULT = 4 # Default: CoSign + TemporalConv1D (conv_type=2) has ~4x downsampling

def compute_min_frames(temporal_conv_type=2, min_encoder_len=2):
    '''Compute minimum input frames from visual encoder architecture.

    For GFSLT-VLP with CoSign backbone + TemporalConv1D:
      conv_type=0 (K3):          ~1x downsampling -> min_frames=2
      conv_type=1 (K5+P2):       ~2x downsampling -> min_frames=4
      conv_type=2 (K5+P2+K5+P2): ~4x downsampling -> min_frames=8

    Args:
        temporal_conv_type: TemporalConv1D conv_type (0, 1, or 2).
        min_encoder_len: Minimum encoder output length (default 2).
    Returns:
        int: minimum input frames.
    '''
    # Approximate downsampling factor from TemporalConv1D
    downsample = {0: 1, 1: 2, 2: 4}.get(temporal_conv_type, 4)
    return downsample * min_encoder_len
