...
def apply_misalignment(keypoint, prev_keypoint, next_keypoint, delta_s_pct, delta_e_pct):
    ...
    '''
    if delta_s_pct == 0 and delta_e_pct == 0: return keypoint
    ...
    min_frames = min_frames = min(T, max(4, round(0.1 * T)))
    ...
    # --- apply truncation -------------------------------------------------
    start, end = head_trunc, T - tail_trunc
...
def get_all_conditions(): # Generate the full list of 41 benchmark conditions
    conditions = [('clean', 0, 0.0, 0.0)]
    for name, (s_sign, e_sign) in CONDITION_TYPES.items():
        if name == 'clean': continue
        for severity in SEVERITY_LEVELS:
            frac = severity / 100.0
            conditions.append((name, severity, s_sign * frac, e_sign * frac))
    return conditions # list of (condition_name, severity_pct, delta_s_pct, delta_e_pct)


def sanity_check(T=100, C=3, V=133): # Print expected output lengths for every condition to verify logic
    ...
    print(f"{'Condition':<32} {'Severity':>7} {'δs':>6} {'δe':>6} {'T_out':>6} {'change':>7}")
    print('-' * 70)
    
    for name, severity, ds, de in get_all_conditions():
        out = apply_misalignment(kp, prev_kp, next_kp, ds, de)
        label = name if severity == 0 else f'{name}@{severity}%'
        change = out.shape[1] - T # change in length from original
        print(f'{label:<32} {severity:>7}% {ds:>+6.2f} {de:>+6.2f} {out.shape[1]:>6} {change:>+7}')
